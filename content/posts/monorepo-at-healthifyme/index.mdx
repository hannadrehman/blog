...
4.__Code reuse:__ The same repository houses common modules, shared libraries, and helper utilities as local packages. These local packages are simple to maintain. we don't need to publish them to private npm repositories, all applications are always running the most recent version of these local packages.
